...
# # from src.email_processing.fetch_emails import email_fetcher  # your EmailFetcher singleton
...
# #             "current_page": 1,
# #             "page_size": 10,
# #             "emails_fetched": 50,      # local pool size (grows with Load More)
# #             "last_fetch_time": None,
# #             "is_fetching": False,
# #             "sender_filter": "",
# #             "subject_filter": "",
# #             "active_category": "Inbox",   # default tab
# #         }
...
# #         st.sidebar.metric("Emails in DB", total_in_db)
# #         st.sidebar.metric("Unread", unread_count)

# #         if st.session_state.last_fetch_time:
# #             st.sidebar.caption(f"⏱ Last refresh: {st.session_state.last_fetch_time}")

# #         # Fetch/Refresh button with loading state
# #         if st.session_state.is_fetching:
# #             st.sidebar.button("🔄 Fetching...", disabled=True)
# #         else:
# #             if st.sidebar.button("🔄 Refresh Emails"):
# #                 self.fetch_from_gmail()

# #         # Info about what refresh does
# #         if total_in_db == 0:
# #             st.sidebar.info("👆 Click Refresh to fetch emails from Gmail for the first time!")
# #         else:
# #             st.sidebar.caption("Refresh fetches new/updated emails from Gmail")

# #         col2, col3 = st.sidebar.columns(2)
# #         if col2.button("🧹 Clear Filters", key="clear_filters_sidebar"):
...
# #             "Emails per page",
# #             [5, 10, 20, 25, 50],
# #             index=[5, 10, 20, 25, 50].index(st.session_state.page_size),
# #         )
# #         if size != st.session_state.page_size:
# #             st.session_state.page_size = size
# #             st.session_state.current_page = 1
# #             st.rerun()

# #         st.sidebar.divider()
# #         st.sidebar.subheader("🔍 Filters")
# #         st.session_state.sender_filter = st.sidebar.text_input(
# #             "Sender", value=st.session_state.sender_filter
# #         )
# #         st.session_state.subject_filter = st.sidebar.text_input(
# #             "Subject", value=st.session_state.subject_filter
# #         )

# #     # ---------------- Gmail Fetch ----------------

# #     def fetch_from_gmail(self):
# #         """Improved Gmail fetching with better status updates"""
...
# #             if is_first_fetch:
# #                 status_container.info("🔄 First time setup - fetching emails...")
# #                 # For first fetch, start from beginning (no page token)
# #                 page_token = None
# #                 batch_size = 100  # Larger batch for initial fetch
# #             else:
# #                 status_container.info("🔄 Checking for new emails...")
# #                 # For subsequent fetches, use the stored token to get new emails
...
# #             st.session_state.current_page = 1  # Reset to first page to see new emails
            
# #             progress_container.progress(1.0)
            
# #             if len(emails) > 0:
# #                 status_container.success(f"✅ Fetched {len(emails)} emails from Gmail!")
# #             else:
# #                 status_container.info("📭 No new emails found")
...
# #             progress_container.empty()
# #             status_container.error(f"❌ Fetch error: {str(e)}")
# #             st.sidebar.error("Check your Gmail API credentials and internet connection.")
# #         finally:
# #             st.session_state.is_fetching = False
# #             # Small delay before rerun to let user see the status
# #             time.sleep(0.5)
# #             st.rerun()
...
# #     def render_emails(self, emails, tab_name):
# #         if not emails:
# #             total_in_db = db.get_total_email_count()
# #             if total_in_db == 0:
# #                 st.info("📭 No emails in database. Click '🔄 Refresh Emails' in sidebar to fetch from Gmail.")
# #             else:
...
# #             expander_title = f"{'📖' if is_read else '📩'} {subject} — {sender}"
            
# #             with st.expander(expander_title):
# #                 st.markdown(f"**From:** {sender}")
# #                 st.markdown(f"**To:** {email.get('to_recipients', 'N/A')}")
...
# #                     st.text_area(
# #                         "Body",
# #                         value=body[:2000] + ("..." if len(body) > 2000 else ""),
# #                         height=200,
# #                         disabled=True,
# #                         key=f"body_{tab_name}_{email['id']}_{i}"
# #                     )
# #                 else:
# #                     st.caption("No body content")

# #                 # Get attachments using a query since there's no get_attachments_by_email_id method
# #                 try:
# #                     db.cursor.execute("SELECT * FROM attachments WHERE email_id = ?", (email["id"],))
# #                     atts = [dict(row) for row in db.cursor.fetchall()]
                    
# #                     if atts:
# #                         st.markdown("📎 **Attachments:**")
# #                         for j, att in enumerate(atts):
# #                             st.caption(f"- {att['filename']} ({att['size']} bytes)")
# #                             if att.get("content_preview"):
# #                                 # Create attachment preview without key
# #                                 with st.expander(f"Preview: {att['filename']}"):
# #                                     st.text(att["content_preview"][:500])
# #                 except Exception as e:
# #                     st.caption(f"Error loading attachments: {e}")

# #                 # Mark as read button with unique key using column and button combination
# #                 if not is_read:
# #                     # Create unique key using email id and timestamp to avoid conflicts
# #                     button_key = f"mark_read_{email['id']}_{tab_name}_{i}"
# #                     if st.button("✅ Mark as Read", key=button_key):
# #                         db.mark_email_as_read(email["id"])
# #                         st.rerun()
...
# #         st.title("📧 Smart Mail Dashboard")
        
# #         # Show helpful instructions based on state
# #         total_emails = db.get_total_email_count()
# #         if total_emails == 0:
# #             st.info("👈 **First time setup:** Click '🔄 Refresh Emails' in the sidebar to fetch your emails from Gmail.")
# #         else:
# #             st.caption(f"📊 {total_emails} emails in database | Click 'Refresh' to sync new emails")

# #         self.render_sidebar()

# #         # Category tabs
# #         tabs = ["Inbox", "Sent", "Drafts", "Promotions", "Important", "No Reply", "Other"]
# #         tab_objects = st.tabs(tabs)
        
# #         for i, tab_name in enumerate(tabs):
# #             with tab_objects[i]:
# #                 st.session_state.active_category = tab_name

# #                 # Use the paginated method from SQLiteManager
# #                 page = st.session_state.current_page
# #                 size = st.session_state.page_size
                
# #                 # Build filters for the database query
# #                 category_filter = tab_name
# #                 sender_filter = st.session_state.sender_filter if st.session_state.sender_filter.strip() else None
# #                 subject_filter = st.session_state.subject_filter if st.session_state.subject_filter.strip() else None
...
# #                     self.render_pagination(total, page, size, tab_name)
# #                     self.render_emails(page_rows, tab_name)
                    
# #                     # Show pagination again at bottom if there are emails
# #                     if page_rows:
# #                         self.render_pagination(total, page, size, f"{tab_name}_bottom")
                    
# #                 except Exception as e:
# #                     st.error(f"Error loading emails for {tab_name}: {e}")
# #                     st.info("💡 Try clicking 'Refresh Emails' in the sidebar to sync from Gmail.")
...
#         # AI Summaries Section (NEW)
#         if summaries:
#             st.markdown("### AI Summaries")
            
#             # Create tabs for different summary types
#             summary_types = list(set(s['summary_type'] for s in summaries))
#             if len(summary_types) > 1:
#                 summary_tabs = st.tabs([f"📄 {stype.title()}" for stype in summary_types])
                
#                 for i, stype in enumerate(summary_types):
#                     with summary_tabs[i]:
#                         summary = next(s for s in summaries if s['summary_type'] == stype)
